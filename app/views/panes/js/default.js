...
function Add(){
	$("#tblPanes").append(
		"<tr id='tr_0'>" +
    	 "<td><input  id='txtIdPanes' name='idPan' type='text' /></td>"+
...
function List()
{
	 $.get('panes/Listar',function(o){
		 ...
		 $('#tblPanes').append("<tr id='tr_"+cont+"'><td>"+this.idPan+"</td><td>"
...
function Show_Fields(ind,tdidPanes,tdDescripcion,tdCantidad,tdUnidadMedida)
{
	if(ind==0){
	tdidPanes.html("<input type='text' id='txtIdUsuario' name='idPan' value='"+tdidPanes.html()+"'/>");
	...
	tdidPanes.html(tdidPanes.children("input[type=text]").val());
...
function Edit()
{
	...
	var tdIdPan = $(id).children("td:nth-child(1)");
    ...
    if(h_fields==0){
    Show_Fields(h_fields,tdIdPan,tdDescripcion,tdCantidad,tdUnidadMedida);
    h_fields=1;
	}else{
	Show_Fields(h_fields,tdIdPan,tdDescripcion,tdCantidad,tdUnidadMedida);
	h_fields=0;
...
function Save(){
	...
	var url= $('#FormPanes').attr('action');
	var data= $('#FormPanes').serialize();
	...
	if(id=="#tr_0"){
		//var pathname = window.location.pathname;
		 ...
		 Insert_Row(this.idPan,this.descripcion,this.cantidad,this.unidadMedida);
...
//			 alert(data);
		 ...
		 Show_Fields(1,tdIdPan,tdDescripcion,tdCantidad,tdUnidadMedida);	
		if(h_save==1){
			$( btn_d ).show();
	        $( btn_s ).hide();
	    h_save=0;
...
function Confirm(){
	...
	$.post('panes/borrar',{'id':id.html()},function(o){
	$( btn_c ).hide();	
	$( id ).remove();
	});
...
function Insert_Row(idPan,descripcion,cantidad,unidadMedida){
  $('#tblPanes').append("<tr><td>"+idPan+"</td><td>"+descripcion+"</td><td>"+cantidad+"</td><td>"+
		  unidadMedida+"</td>" +
		   +"<td><div class='tabledit-toolbar btn-toolbar' style='text-align: left;'>"+
			"<div class='btn-group btn-group-sm' style='float: none;'>"+
