...
var h_fields=0;
function Add(){
	$("#tblMermaFrutas").append(
		"<tr id='tr_0'>" +
    	 "<td><input  id='txtIdMermaFruta' name='idMermaFruta' type='text' /></td>"+
		  "<td><input id='txtFruta' name='fruta' type='text'/></td>"+
...
function List(){
	 $.get('../mermafrutas/Listar',function(o){
		 
		 var cont=1;
		
		 $(jQuery.parseJSON(o)).each(function() {  
		
		 $('#tblMermaFrutas').append("<tr id='tr_"+cont+"'><td>"+this.idMermaFruta+"</td><td>"
		+this.fruta+"</td><td>"+this.fechaCreacion+"</td><td>"+this.cantidad+"</td>" +
...
function Show_Fields(ind,tdIdMermaFruta,tdpan,tdCantidad,tdFechaCreacion)
{
	if(ind==0){
	tdIdMermaFruta.html("<input type='text' id='txtIdUsuario' name='idMermaFruta' value='"+tdIdMermaFruta.html()+"'/>");
	...
	tdIdMermaFruta.html(tdIdMermaFruta.children("input[type=text]").val());
...
function Edit()
{
	...
	var tdIdMermaFruta = $(id).children("td:nth-child(1)");
    ...
    if(h_fields==0){
    Show_Fields(h_fields,tdIdMermaFruta,tdPan,tdCantidad,tdFechaCreacion);
    h_fields=1;
	}else{
	Show_Fields(h_fields,tdIdMermaFruta,tdPan,tdCantidad,tdFechaCreacion);
	h_fields=0;
...
function Save(){
	...
	var url= $('#FormMermaFrutas').attr('action');
	var data= $('#FormMermaFrutas').serialize();
	...
	if(id=="#tr_0"){
		//var pathname = window.location.pathname;
		 ...
		 Insert_Row(this.idMermaFruta,this.fruta,this.fechaCreacion,this.cantidad);
...
//			 alert(data);
		 ...
		 Show_Fields(1,tdidMermaFruta,tdPan,tdFechaCreacion,tdCantidad);	
		if(h_save==1){
			$( btn_d ).show();
	        $( btn_s ).hide();
	    h_save=0;
...
function Confirm(){
	...
	$.post('frutas/borrar',{'id':id.html()},function(o){
	$( btn_c ).hide();	
	$( id ).remove();
	});
...
function Insert_Row(idMermaFruta,fruta,fechaCreacion,cantidad){
  $('#tblMermaFrutas').append("<tr><td>"+idMermaFruta+"</td><td>"+fruta+"</td><td>"+fechaCreacion+"</td><td>"+
		  cantidad+"</td>" +
		   +"<td><div class='tabledit-toolbar btn-toolbar' style='text-align: left;'>"+
			"<div class='btn-group btn-group-sm' style='float: none;'>"+
