...
var h_fields=0;
function Add(){
	$("#tblMermaPanes").append(
		"<tr id='tr_0'>" +
    	 "<td><input  id='txtIdMermaPan' name='idMermaPan' type='text' /></td>"+
		  "<td><input id='txtPan' name='pan' type='text'/></td>"+
...
function List(){
	 $.get('../mermapanes/Listar',function(o){
		 
		 var cont=1;
		
		 $(jQuery.parseJSON(o)).each(function() {  
		
		 $('#tblMermaPanes').append("<tr id='tr_"+cont+"'><td>"+this.idMermaPan+"</td><td>"
		+this.pan+"</td><td>"+this.fechaCreacion+"</td><td>"+this.cantidad+"</td>" +
...
function Show_Fields(ind,tdIdMermaPan,tdpan,tdCantidad,tdFechaCreacion)
{
	if(ind==0){
	tdIdMermaPan.html("<input type='text' id='txtIdUsuario' name='idMermaPan' value='"+tdIdMermaPan.html()+"'/>");
	...
	tdIdMermaPan.html(tdIdMermaPan.children("input[type=text]").val());
...
function Edit()
{
	...
	var tdIdMermaPan = $(id).children("td:nth-child(1)");
    ...
    if(h_fields==0){
    Show_Fields(h_fields,tdIdMermaPan,tdPan,tdCantidad,tdFechaCreacion);
    h_fields=1;
	}else{
	Show_Fields(h_fields,tdIdMermaPan,tdPan,tdCantidad,tdFechaCreacion);
	h_fields=0;
...
function Save(){
	...
	var url= $('#FormMermaPanes').attr('action');
	var data= $('#FormMermaPanes').serialize();
	...
	if(id=="#tr_0"){
		//var pathname = window.location.pathname;
		 ...
		 Insert_Row(this.idMermaPan,this.pan,this.fechaCreacion,this.cantidad);
...
//			 alert(data);
		 ...
		 Show_Fields(1,tdIdMermaPan,tdPan,tdFechaCreacion,tdCantidad);	
		if(h_save==1){
			$( btn_d ).show();
	        $( btn_s ).hide();
	    h_save=0;
...
function Insert_Row(idMermaPan,pan,fechaCreacion,cantidad){
  $('#tblMermaPanes').append("<tr><td>"+idMermaPan+"</td><td>"+pan+"</td><td>"+fechaCreacion+"</td><td>"+
		  cantidad+"</td>" +
		   +"<td><div class='tabledit-toolbar btn-toolbar' style='text-align: left;'>"+
			"<div class='btn-group btn-group-sm' style='float: none;'>"+
