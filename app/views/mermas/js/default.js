...
function Add(){
	$("#tblFrutas").append(
		"<tr id='tr_0'>" +
    	 "<td><input  id='txtIdFruta' name='idFruta' type='text' /></td>"+
...
function List()
{
	 $.get('frutas/Listar',function(o){
		 ...
		 $('#tblFrutas').append("<tr id='tr_"+cont+"'><td>"+this.idFruta+"</td><td>"
...
function Show_Fields(ind,tdIdFruta,tdDescripcion,tdCantidad,tdUnidadMedida)
{
	if(ind==0){
	tdIdFruta.html("<input type='text' id='txtIdUsuario' name='idFruta' value='"+tdIdFruta.html()+"'/>");
...
function Edit()
{
	...
	var tdIdFruta = $(id).children("td:nth-child(1)");
    ...
    if(h_fields==0){
    Show_Fields(h_fields,tdIdFruta,tdDescripcion,tdCantidad,tdUnidadMedida);
    h_fields=1;
	}else{
	Show_Fields(h_fields,tdIdFruta,tdDescripcion,tdCantidad,tdUnidadMedida);
	h_fields=0;
...
function Save(){
	...
	var url= $('#FormFrutas').attr('action');
	var data= $('#FormFrutas').serialize();
	...
	if(id=="#tr_0"){
		//var pathname = window.location.pathname;
		 ...
		 Insert_Row(this.idFruta,this.descripcion,this.cantidad,this.unidadMedida);
...
//			 alert(data);
		 ...
		 Show_Fields(1,tdIdFruta,tdDescripcion,tdCantidad,tdUnidadMedida);	
		if(h_save==1){
			$( btn_d ).show();
	        $( btn_s ).hide();
	    h_save=0;
...
function Insert_Row(idFruta,descripcion,cantidad,unidadMedida){
  $('#tblFrutas').append("<tr><td>"+idFruta+"</td><td>"+descripcion+"</td><td>"+cantidad+"</td><td>"+
		  unidadMedida+"</td>" +
		   +"<td><div class='tabledit-toolbar btn-toolbar' style='text-align: left;'>"+
			"<div class='btn-group btn-group-sm' style='float: none;'>"+
