...
var h_fields=0;
function Add(){
	$("#tblMermaCarnes").append(
		"<tr id='tr_0'>" +
    	 "<td><input  id='txtIdMermaCarne' name='idMermaCarne' type='text' /></td>"+
		  "<td><input id='txtCarne' name='carne' type='text'/></td>"+
...
function List(){
	 $.get('../mermacarnes/Listar',function(o){
		 
		 var cont=1;
		
		 $(jQuery.parseJSON(o)).each(function() {  
		
		 $('#tblMermaCarnes').append("<tr id='tr_"+cont+"'><td>"+this.idMermaCarne+"</td><td>"
		+this.carne+"</td><td>"+this.fechaCreacion+"</td><td>"+this.cantidad+"</td>" +
...
function Insert_Row(idMermaCarne,carne,fechaCreacion,cantidad){
  $('#tblMermaCarnes').append("<tr><td>"+idMermaCarne+"</td><td>"+carne+"</td><td>"+fechaCreacion+"</td><td>"+
		  cantidad+"</td>" +
		   +"<td><div class='tabledit-toolbar btn-toolbar' style='text-align: left;'>"+
			"<div class='btn-group btn-group-sm' style='float: none;'>"+
