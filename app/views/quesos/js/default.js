...
function Add(){
	$("#tblQuesos").append(
		"<tr id='tr_0'>" +
    	 "<td><input  id='txtIdQueso' name='idQueso' type='text' /></td>"+
...
function List()
{
	 $.get('quesos/Listar',function(o){
		 ...
		 $('#tblQuesos').append("<tr id='tr_"+cont+"'><td>"+this.idQueso+"</td><td>"
...
function Show_Fields(ind,tdIdQueso,tdDescripcion,tdCantidad,tdUnidadMedida)
{
	if(ind==0){
	tdIdQueso.html("<input type='text' id='txtIdUsuario' name='IdQueso' value='"+tdIdQueso.html()+"'/>");
	...
	tdIdQueso.html(tdIdQueso.children("input[type=text]").val());
...
function Edit()
{
	...
	var tdIdQueso = $(id).children("td:nth-child(1)");
    ...
    if(h_fields==0){
    Show_Fields(h_fields,tdIdQueso,tdDescripcion,tdCantidad,tdUnidadMedida);
    h_fields=1;
	}else{
	Show_Fields(h_fields,tdIdQueso,tdDescripcion,tdCantidad,tdUnidadMedida);
	h_fields=0;
...
function Save(){
	...
	var url= $('#FormQuesos').attr('action');
	var data= $('#FormQuesos').serialize();
	...
	if(id=="#tr_0"){
		//var pathname = window.location.pathname;
		 ...
		 Insert_Row(this.idQueso,this.descripcion,this.cantidad,this.unidadMedida);
...
//			 alert(data);
		 ...
		 Show_Fields(1,tdIdQueso,tdDescripcion,tdCantidad,tdUnidadMedida);	
		if(h_save==1){
			$( btn_d ).show();
	        $( btn_s ).hide();
	    h_save=0;
...
function Confirm(){
	...
	$.post('quesos/borrar',{'id':id.html()},function(o){
	$( btn_c ).hide();	
	$( id ).remove();
	});
...
function Insert_Row(idQueso,descripcion,cantidad,unidadMedida){
  $('#tblQuesos').append("<tr><td>"+idQueso+"</td><td>"+descripcion+"</td><td>"+cantidad+"</td><td>"+
		  unidadMedida+"</td>" +
		   +"<td><div class='tabledit-toolbar btn-toolbar' style='text-align: left;'>"+
			"<div class='btn-group btn-group-sm' style='float: none;'>"+
