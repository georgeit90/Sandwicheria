...
function List()
{
	 $.get('vegetales/Listar',function(o){
		 ...
		 $('#tblVegetales').append("<tr id='tr_"+cont+"'><td>"+this.idVegetal+"</td><td>"
...
function Show_Fields(ind,tdIdVegetal,tdDescripcion,tdCantidad,tdUnidadMedida)
{
	if(ind==0){
	tdIdVegetal.html("<input type='text' id='txtIdUsuario' name='idVegetal' value='"+tdIdVegetal.html()+"'/>");
	...
	tdIdVegetal.html(tdIdVegetal.children("input[type=text]").val());
...
function Edit()
{
	...
	var tdIdVegetal = $(id).children("td:nth-child(1)");
    ...
    if(h_fields==0){
    Show_Fields(h_fields,tdIdVegetal,tdDescripcion,tdCantidad,tdUnidadMedida);
    h_fields=1;
	}else{
	Show_Fields(h_fields,tdIdVegetal,tdDescripcion,tdCantidad,tdUnidadMedida);
	h_fields=0;
...
function Save(){
	...
	var url= $('#FormVegetales').attr('action');
	var data= $('#FormVegetales').serialize();
	...
	if(id=="#tr_0"){
		//var pathname = window.location.pathname;
		 ...
		 Insert_Row(this.idVegetal,this.descripcion,this.cantidad,this.unidadMedida);
...
//			 alert(data);
		 ...
		 Show_Fields(1,tdIdVegetal,tdDescripcion,tdCantidad,tdUnidadMedida);	
		if(h_save==1){
			$( btn_d ).show();
	        $( btn_s ).hide();
	    h_save=0;
