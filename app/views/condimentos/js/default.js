...
var h_fields=0;
function Add(){
	$("#tblCondimentos").append(
		"<tr id='tr_0'>" +
    	 "<td><input  id='txtIdCondimento' name='idCondimento' type='text' /></td>"+
...
function List()
{
	 $.get('condimentos/Listar',function(o){
		 ...
		 $('#tblCondimentos').append("<tr id='tr_"+cont+"'><td>"+this.idCondimento+"</td><td>"
...
function Show_Fields(ind,tdIdCondimento,tdDescripcion,tdCantidad,tdUnidadMedida)
{
	if(ind==0){
	tdIdCondimento.html("<input type='text' id='txtIdCondimento' name='idCondimento' value='"+tdIdCondimento.html()+"'/>");
	...
	tdIdFruta.html(tdIdFruta.children("input[type=text]").val());
...
function Edit()
{
	...
	var tdIdCondimento = $(id).children("td:nth-child(1)");
    ...
    if(h_fields==0){
    Show_Fields(h_fields,tdIdCondimento,tdDescripcion,tdCantidad,tdUnidadMedida);
    h_fields=1;
	}else{
	Show_Fields(h_fields,tdIdCondimento,tdDescripcion,tdCantidad,tdUnidadMedida);
	h_fields=0;
...
function Save(){
	...
	var url= $('#FormCondimentos').attr('action');
	var data= $('#FormCondimentos').serialize();
	...
	if(id=="#tr_0"){
		//var pathname = window.location.pathname;
		 ...
		 Insert_Row(this.idCondimento,this.descripcion,this.cantidad,this.unidadMedida);
...
//			 alert(data);
		 ...
		 Show_Fields(1,tdIdCondimento,tdDescripcion,tdCantidad,tdUnidadMedida);	
		if(h_save==1){
			$( btn_d ).show();
	        $( btn_s ).hide();
	    h_save=0;
...
function Confirm(){
	...
	$.post('condimentos/borrar',{'id':id.html()},function(o){
	$( btn_c ).hide();	
	$( id ).remove();
	});
...
function Insert_Row(idFruta,descripcion,cantidad,unidadMedida){
  $('#tblCondimentos').append("<tr><td>"+idFruta+"</td><td>"+descripcion+"</td><td>"+cantidad+"</td><td>"+
		  unidadMedida+"</td>" +
		   +"<td><div class='tabledit-toolbar btn-toolbar' style='text-align: left;'>"+
			"<div class='btn-group btn-group-sm' style='float: none;'>"+
